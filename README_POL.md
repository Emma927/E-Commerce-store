...
Dane użytkownika wybrane po ID - GET `/users/:id`
```
> Token JWT jest używany po stronie frontendu wyłącznie do kontrolowania, czy fetch powinien zostać wykonany (enabled: !!token). Fake API nie weryfikuje tokena, więc w praktyce do pobrania danych użytkownika wystarczy samo id.
...
> API zwraca token JWT (symulowany) i zapisuje go w lokalnej pamięci przeglądarki (localStorage).
Token ma poprawny format JWT, ale nie jest podpisany ani weryfikowany po stronie backendu, więc jego użycie nie zapewnia bezpieczeństwa i służy wyłącznie do celów demonstracyjnych.
...
- Dzięki temu, że w docker-compose.yml wolumeny są zdefiniowane jako bind mounty na WSL2, polecenie down usuwa tylko kontenery (system operacyjny, przeglądarki), ale pozostawia kod źródłowy i folder node_modules bezpiecznie na dysku (EXT4).
...
2.1.1. Testy i instalacja zależności
...
2.1.2 Build frontendu

- Kompilacja aplikacji React (tworzenie katalogu dist)

2.1.3. Serwowanie aplikacji przez Nginx
...
2.3 Testy orbazu produkcyjnego
